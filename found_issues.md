...
## 5. Pre-existing `R CMD check` warnings / notes (baseline cleanup)

These predate this work and are unrelated to the three defects or the new tests.
`devtools::check()` reports **0 errors**, with the following baseline:

- **WARNING — invalid license file pointer.** `DESCRIPTION` declares
  `MIT + file LICENSE`, but the repo ships `LICENSE.md`. Fix: add a `LICENSE`
  file (the two-line MIT year/holder stub) or change the field to
  `MIT + file LICENSE.md` per the R packaging convention.
- **WARNING — undeclared `actuar`.** `R/parametric_distribution_model.R`
  (`.sample_from_fitdist()`) calls `requireNamespace("actuar")` but `actuar` is
  not in `DESCRIPTION` Suggests. Fix: add `actuar` to Suggests.
- **NOTE — non-standard top-level files:** `data_raw/`, `examples/`,
  `lm_prediction_interval_test.R`, `package_test.R`. Fix: move scratch scripts
  out of the package root or add them to `.Rbuildignore`.
- **NOTE — S3 generic/method consistency.** Every `predict.*` method uses
  ...
  first formal). Fix: rename the first arg to `object` (and update bodies), or
  register the methods accordingly.
